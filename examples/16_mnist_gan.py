...
This experiment creates a Generative Adversarial Network (GAN) -https://arxiv.org/abs/1406.2661-
that generates characters (numbers) alla MNIST. The experiment uses analog layers to implement
the network. Also, while training it plots the different characters generated by the generator
neural network vs. those used to train the discriminator neural network.
Also, for every epoch, it shows the loss for the generator and discriminator.
"""
...
from aihwkit.simulator.presets import MixedPrecisionPCMPreset
...
def training_loop(
    ...
    Args:
        ...
        gen_opt: analog model optimizer for the generator
        disc_opt: analog model optimizer for the discriminator
        criterion: criterion to compute loss
        dataloader: Data set to train and evaluate the models
